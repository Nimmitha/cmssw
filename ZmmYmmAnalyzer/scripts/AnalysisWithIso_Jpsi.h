...
#ifndef AnalysisWithIso_Jpsi_h
#define AnalysisWithIso_Jpsi_h
...
public:
  ...
  AnalysisWithIso_Jpsi(TTree *tree = 0);
  virtual ~AnalysisWithIso_Jpsi();
...
#ifdef AnalysisWithIso_Jpsi_cxx
AnalysisWithIso_Jpsi::AnalysisWithIso_Jpsi(TTree *tree) : fChain(0) {
  // if parameter tree is not specified (or zero), connect the file
  // used to generate this class and read the Tree.
  if (tree == 0) {
    // TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject("../outputs/crab_TTree_13TeV_mmmm_Run3.root");
    // TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject("../inputFiles/mc_zmmjpsimm_v1/mc_zmmjpsimm_v1.root");
    TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject(
        "../inputFiles/mmmmJesse/crab_TTree_13TeV_mmmm_Run3.root");
...
AnalysisWithIso_Jpsi::~AnalysisWithIso_Jpsi() {
  if (!fChain)
    return;
  delete fChain->GetCurrentFile();
}

Int_t AnalysisWithIso_Jpsi::GetEntry(Long64_t entry) {
  // Read contents of entry.
  if (!fChain)
    return 0;
  return fChain->GetEntry(entry);
}
Long64_t AnalysisWithIso_Jpsi::LoadTree(Long64_t entry) {
...
void AnalysisWithIso_Jpsi::Init(TTree *tree) {
...
Bool_t AnalysisWithIso_Jpsi::Notify() {
...
void AnalysisWithIso_Jpsi::Show(Long64_t entry) {
...
Int_t AnalysisWithIso_Jpsi::Cut(Long64_t entry) {
  // This function may be called from Loop.
  // returns  1 if entry is accepted.
  // returns -1 otherwise.
  return 1;
}
#endif  // #ifdef AnalysisWithIso_Jpsi_cxx
