...
#include "ZmmYeeAnalyzer/miniAODzmmyee/plugins/miniAODzmmyee.h"
...
miniAODzmmyee::miniAODzmmyee(const edm::ParameterSet& iConfig)
...
void miniAODzmmyee::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using std::vector;
  using namespace edm;
  ...
  if (!theB.isValid()) {
    edm::LogWarning("miniAODzmmyee") << "no Transient Track in event";
    return;
  }

  if (!thePATElectronHandle.isValid()) {
    edm::LogWarning("miniAODzmmyee") << "no pat::Electrons in event";
    return;
  }
  if (!thePATMuonHandle.isValid()) {
    edm::LogWarning("miniAODzmmyee") << "no pat::Muons in event";
    return;
  }
  if (!triggerBits.isValid()) {
    edm::LogWarning("miniAODzmmyee") << "no Trigger path in event";
    return;
  }
  if (!triggerObjects.isValid()) {
    edm::LogWarning("miniAODzmmyee") << "no Trigger Object in event";
    return;
  }
...
void miniAODzmmyee::endJob() {
  tree_->GetDirectory()->cd();
  tree_->Write();
}

//define this as a plug-in
DEFINE_FWK_MODULE(miniAODzmmyee);
