...
//std::cout << "\tTrigger objectisBoth:  pt " << obj.pt() << ", eta " << obj.eta() << ", phi " << obj.phi() << std::endl;
  ...
  for(View<pat::Muon>::const_iterator iMuon1 = thePATMuonHandle->begin(); iMuon1 != thePATMuonHandle->end(); ++iMuon1) {
	...
	for(View<pat::Muon>::const_iterator iMuon4 = iMuon3+1; iMuon4 != thePATMuonHandle->end(); ++iMuon4) {
	  ...
	  if (!(abs (iMuon1->charge() )==1)) continue;
	  if (!(abs (iMuon2->charge() )==1)) continue;
	  if (!(abs (iMuon3->charge() )==1)) continue;
	  if (!(abs (iMuon4->charge() )==1)) continue;
	  ...
	  //preselection ptcut 
	  if (iMuon1->pt()<2.0)continue;
	  if (iMuon2->pt()<2.0)continue;
	  if (iMuon3->pt()<2.0)continue;
	  if (iMuon4->pt()<2.0)continue;
	  TrackRef glbTrackP1;	  
	  TrackRef glbTrackP2;	  
	  TrackRef glbTrackM1;	  
	  TrackRef glbTrackM2;	  
	  ...
	  if(!(glbTrackM1->quality(reco::TrackBase::highPurity))) continue;
	  if(!(glbTrackP1->quality(reco::TrackBase::highPurity))) continue;	 
	  if(!(glbTrackM2->quality(reco::TrackBase::highPurity))) continue;
	  if(!(glbTrackP2->quality(reco::TrackBase::highPurity))) continue;	 
  ...
  B_U_TriggerPt1->clear();B_U_TriggerEta1->clear();B_U_TriggerPhi1->clear();
  B_U_TriggerPt2->clear();B_U_TriggerEta2->clear();B_U_TriggerPhi2->clear();
  B_U_TriggerPt3->clear();B_U_TriggerEta3->clear();B_U_TriggerPhi3->clear();
  B_U_TriggerPt4->clear();B_U_TriggerEta4->clear();B_U_TriggerPhi4->clear();
  B_U_TriggerPt5->clear();B_U_TriggerEta5->clear();B_U_TriggerPhi5->clear();
  
  B_J1_mass->clear(); B_J1_px->clear();   B_J1_py->clear();  B_J1_pz->clear();
  B_J1_pt->clear();   B_J1_eta->clear();  B_J1_phi->clear();
  B_J1_rapidity->clear(); 
  B_J2_mass->clear(); B_J2_px->clear();   B_J2_py->clear();  B_J2_pz->clear();
  B_J2_pt->clear();   B_J2_eta->clear();  B_J2_phi->clear();
  B_J2_rapidity->clear(); 
  B_J3_mass->clear(); B_J3_px->clear();   B_J3_py->clear();  B_J3_pz->clear();
  B_J3_pt->clear();   B_J3_eta->clear();  B_J3_phi->clear();
  B_J3_rapidity->clear(); 
  B_J4_mass->clear(); B_J4_px->clear();   B_J4_py->clear();  B_J4_pz->clear();
  B_J4_pt->clear();   B_J4_eta->clear();  B_J4_phi->clear();
  B_J4_rapidity->clear(); 
  
  B_J1_VtxPx->clear();
  B_J1_VtxPy->clear();  B_J1_VtxPz->clear();
  B_J1_VtxPt->clear();   B_J1_VtxEta->clear();  B_J1_VtxPhi->clear(); 
  B_J1_VtxRapidity->clear(); B_J1_VtxMass->clear();
  B_J1_PVx->clear(); B_J1_PVy->clear(); B_J1_PVz->clear();
  B_J1_PVxError->clear(); B_J1_PVyError->clear(); B_J1_PVzError->clear();
  B_J2_VtxPx->clear();
  B_J2_VtxPy->clear();  B_J2_VtxPz->clear();
  B_J2_VtxPt->clear();   B_J2_VtxEta->clear();  B_J2_VtxPhi->clear(); 
  B_J2_VtxRapidity->clear(); B_J2_VtxMass->clear();
  B_J2_PVx->clear(); B_J2_PVy->clear(); B_J2_PVz->clear();
  B_J2_PVxError->clear(); B_J2_PVyError->clear(); B_J2_PVzError->clear();
  B_J3_VtxPx->clear();
  B_J3_VtxPy->clear();  B_J3_VtxPz->clear();
  B_J3_VtxPt->clear();   B_J3_VtxEta->clear();  B_J3_VtxPhi->clear(); 
  B_J3_VtxRapidity->clear(); B_J3_VtxMass->clear();
  B_J3_PVx->clear(); B_J3_PVy->clear(); B_J3_PVz->clear();
  B_J3_PVxError->clear(); B_J3_PVyError->clear(); B_J3_PVzError->clear();
  B_J4_VtxPx->clear();
  B_J4_VtxPy->clear();  B_J4_VtxPz->clear();
  B_J4_VtxPt->clear();   B_J4_VtxEta->clear();  B_J4_VtxPhi->clear(); 
  B_J4_VtxRapidity->clear(); B_J4_VtxMass->clear();
  B_J4_PVx->clear(); B_J4_PVy->clear(); B_J4_PVz->clear();
  B_J4_PVxError->clear(); B_J4_PVyError->clear(); B_J4_PVzError->clear();

  B_Mu1_px->clear();  B_Mu1_py->clear();  B_Mu1_pz->clear(); B_Mu1_charge->clear();
  B_Mu1_pt->clear();  B_Mu1_eta->clear(); B_Mu1_phi->clear();
  B_Mu1_soft->clear(); B_Mu1_tight->clear(); B_Mu1_loose->clear();
  B_Mu1_IsoTrack->clear(); B_Mu1_IsoHcal->clear(); B_Mu1_IsoEcal->clear(); B_Mu1_IsoCalo->clear();
  B_Mu2_px->clear();  B_Mu2_py->clear();  B_Mu2_pz->clear(); B_Mu2_charge->clear();
  B_Mu2_pt->clear();  B_Mu2_eta->clear(); B_Mu2_phi->clear(); 
  B_J1_VtxProb->clear(); B_J2_VtxProb->clear(); B_J3_VtxProb->clear(); B_J4_VtxProb->clear();
  B_Mu2_soft->clear(); B_Mu2_tight->clear(); B_Mu2_loose->clear();
  B_Mu2_IsoTrack->clear(); B_Mu2_IsoHcal->clear(); B_Mu2_IsoEcal->clear(); B_Mu2_IsoCalo->clear();
  B_Mu3_px->clear();  B_Mu3_py->clear();  B_Mu3_pz->clear(); 
  B_Mu3_pt->clear();  B_Mu3_eta->clear(); B_Mu3_phi->clear();
  B_Mu3_soft->clear(); B_Mu3_tight->clear(); B_Mu3_loose->clear(); B_Mu3_charge->clear();
  B_Mu3_IsoTrack->clear(); B_Mu3_IsoHcal->clear(); B_Mu3_IsoEcal->clear(); B_Mu3_IsoCalo->clear();
  B_Mu4_px->clear();  B_Mu4_py->clear();  B_Mu4_pz->clear(); 
  B_Mu4_pt->clear();  B_Mu4_eta->clear(); B_Mu4_phi->clear();
  B_Mu4_soft->clear(); B_Mu4_tight->clear(); B_Mu4_loose->clear();B_Mu4_charge->clear();
  B_Mu4_IsoTrack->clear(); B_Mu4_IsoHcal->clear(); B_Mu4_IsoEcal->clear(); B_Mu4_IsoCalo->clear();
  B_J_xyP1->clear(); B_J_xyM1->clear(); B_J_zP1->clear(); B_J_zM1->clear();
  B_J_xyP2->clear(); B_J_xyM2->clear(); B_J_zP2->clear(); B_J_zM2->clear();
  B_Mu1_PaperIsoTrackRF04->clear();
  B_Mu1_PaperIsoTrackRF03->clear();
  B_Mu2_PaperIsoTrackRF04->clear();                                                                                                                                                           B_Mu2_PaperIsoTrackRF03->clear();
  B_Mu3_PaperIsoTrackRF04->clear();                                                                                                                                                           B_Mu3_PaperIsoTrackRF03->clear();
  B_Mu4_PaperIsoTrackRF04->clear();                                                                                                                                                           B_Mu4_PaperIsoTrackRF03->clear();
  ...
  mu1mC2->clear();
  mu1mNHits->clear(); mu1mNPHits->clear();
  mu1pC2->clear();
  mu1pNHits->clear(); mu1pNPHits->clear();
  mu2mC2->clear();
  mu2mNHits->clear(); mu2mNPHits->clear();
  mu2pC2->clear();
  mu2pNHits->clear(); mu2pNPHits->clear();
  
  B_M1_pt->clear(); B_M1_eta->clear(); B_M1_phi->clear();
  B_M1_px->clear(); B_M1_px->clear(); B_M1_pz->clear();
  B_M2_pt->clear(); B_M2_eta->clear(); B_M2_phi->clear();
  B_M2_px->clear(); B_M2_px->clear(); B_M2_pz->clear();
  B_M3_pt->clear(); B_M3_eta->clear(); B_M3_phi->clear();
  B_M3_px->clear(); B_M3_px->clear(); B_M3_pz->clear();
  B_M4_pt->clear(); B_M4_eta->clear(); B_M4_phi->clear();
  B_M4_px->clear(); B_M4_px->clear(); B_M4_pz->clear();
  
  //Gen muon
  B_J_GenMuonPt->clear(); B_J_GenMuonEta->clear(); B_J_GenMuonPhi->clear();
  B_Z_GenMuonPt->clear(); B_Z_GenMuonEta->clear(); B_Z_GenMuonPhi->clear();
...
miniAODmuons::beginJob()
{
  ...
  tree_->Branch("B_Mu2_PaperIsoTrackRF03", &B_Mu2_PaperIsoTrackRF03);                                                                                                                                               tree_->Branch("B_Mu2_PaperIsoTrackRF04", &B_Mu2_PaperIsoTrackRF04); 
  ...
  tree_->Branch("B_Mu3_PaperIsoTrackRF03", &B_Mu3_PaperIsoTrackRF03);                                                                                                                                               tree_->Branch("B_Mu3_PaperIsoTrackRF04", &B_Mu3_PaperIsoTrackRF04); 
  ...
  tree_->Branch("B_Mu4_PaperIsoTrackRF03", &B_Mu4_PaperIsoTrackRF03);                                                                                                                                               tree_->Branch("B_Mu4_PaperIsoTrackRF04", &B_Mu4_PaperIsoTrackRF04); 
