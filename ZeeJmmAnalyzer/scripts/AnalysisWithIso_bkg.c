#define AnalysisWithIso_bkg_cxx
#include "AnalysisWithIso_bkg.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void AnalysisWithIso_bkg::Loop() {
  //   In a ROOT session, you can do:
  //      root> .L AnalysisWithIso_bkg.C
  //      root> AnalysisWithIso_bkg t
  ...
  ofstream myfile1;
  myfile.precision(17);
  myfile.open("ZeeJmm_candidatesE.dat");
  myfile1.open("ZeeJmm_Multiples.dat");
  TFile *fFile = new TFile("ZeeJmm_andidatesE.root", "recreate");
  ...
  vector<bool> *B_Z_mvaIsoWP90_1_b;
  vector<bool> *B_Z_mvaIsoWP90_2_b;
  ...
  b_B_Z_mvaIsoWP90_1 = 0;
  b_B_Z_mvaIsoWP90_2 = 0;
  ...
  fTree->Branch("B_J_mass", &B_J_mass_b);
  ...
  fTree->Branch("B_Z_mvaIsoWP90_1", &b_B_Z_mvaIsoWP90_1);
  fTree->Branch("B_Z_mvaIsoWP90_2", &b_B_Z_mvaIsoWP90_2);
  fTree->Branch("B_J_soft1", &b_B_J_soft1);
  fTree->Branch("B_J_soft2", &b_B_J_soft2);
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    for (int i = 0; i < Event->size(); i++) {
      ...
      // cout << "TriggerPath: " << Mu_TriggerPath->at(i) << endl;
      // cout << "TriggerPath1: " << Mu_TriggerPath1->at(i) << endl;
      // cout << "B_J_soft1: " << B_J_soft1->at(i) << endl;
      // cout << "B_J_mass: " << B_J_mass->at(i) << endl;

      //cut 2 Trigger
      if (Ele_TriggerPath->at(i) < 1) {
        continue;
      }
      ...
      if (B_J_soft1->at(i) < 1 || B_J_soft2->at(i) < 1)
        continue;

      //	if (  B_Z_looseHEEPwp2->at(i)   < 1) continue;
      //cout<<B_Z_mvaIsoWP90_1->at(i)<<endl;
      //cout<<B_Z_looseHZZwp1->at(i)<<endl;

      if (B_Z_mvaIsoWP90_1->at(i) < 1 && B_Z_mvaIsoWP90_2->at(i) < 1)
        continue;
      //if (  B_Z_mvaIsoWP90_1->at(i)   < 1) continue;
      //if (  B_Z_mvaIsoWP90_2->at(i)   < 1) continue;
      //     if (  B_Z_looseHZZwp1->at(i)   < 1) continue;
      //     if (  B_Z_looseHZZwp2->at(i)   < 1) continue;
      //     if (  B_Z_looseCutBase1->at(i)   < 1) continue;
      //     if (  B_Z_looseCutBase2->at(i)   < 1) continue;
      ...
      int UpsVtx = 0;
      if (B_Z_VtxProb->at(i) > 0.005) {
        if (B_J_VtxProb->at(i) > 0.005) {
          UpsVtx++;
        }
      }
      if (UpsVtx < 1)
        continue;
      if (FourL_VtxProb->at(i) < 0.005)
        continue;
      ...
      if ((B_J_mass->at(i) > 2.8 && B_J_mass->at(i) < 4.0)) {
        UpsMass++;
        check1++;
      }
      ...
      Ups1_mass = B_Z_mass->at(i);
      Ups_VtxProb1 = B_Z_VtxProb->at(i);
      Ups_Pt1 = B_Z_pt->at(i);
      Ups_Rapidity1 = B_Z_rapidity->at(i);
      Ups_Phi1 = B_Z_phi->at(i);
      Ups2_mass = B_J_mass->at(i);
      Ups_VtxProb2 = B_J_VtxProb->at(i);
      Ups_Pt2 = B_J_pt->at(i);
      Ups_Rapidity2 = B_J_rapidity->at(i);
      Ups_Phi2 = B_J_phi->at(i);
...
//Calculation of Isolation 1
      ...
      float B_Mu_IsoTrackCorr1 = B_Z_trackiso1->at(i);
      if (DeltaR12<0.3) {
...
*/
   ...
   */
      ...
      //Dilepton mass cut 5
      if (Ups2_mass < 2.8 || Ups2_mass > 4.0)
        continue;
      if (Ups1_mass < 70.0 || Ups1_mass > 110)
        continue;
      //if (Ups1_mass < 80.0 || Ups1_mass>100) continue;
      ...
      B_Z_mass_b->push_back(B_Z_mass->at(i));
      B_J_mass_b->push_back(B_J_mass->at(i));
      B_TriggerDelta_pt_b->push_back(B_Z_TriggerPt1->at(i) - B_Z_pt1->at(i));
