...
public:
  ...
  vector<float> *B_Z_TriggerPath;
  vector<float> *B_Z_TriggerPath1;
  ...
  TBranch *b_B_Z_TriggerPath;   //!
  TBranch *b_B_Z_TriggerPath1;  //!
...
AnalysisWithIso::AnalysisWithIso(TTree *tree) : fChain(0) {
  ...
  if (tree == 0) {
    TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject("../inputFiles/EGamma/crab_TTree_13TeV_zeeymm_Run3.root");

    if (!f || !f->IsOpen()) {
      f = new TFile("../inputFiles/EGamma/crab_TTree_13TeV_zeeymm_Run3.root");
    }
...
void AnalysisWithIso::Init(TTree *tree) {
  ...
  fChain->SetBranchAddress("B_Z_TriggerPath", &B_Z_TriggerPath, &b_B_Z_TriggerPath);
  fChain->SetBranchAddress("B_Z_TriggerPath1", &B_Z_TriggerPath1, &b_B_Z_TriggerPath1);
