// -*- C++ -*-
//
// Package:    ZeeYmmAnalyzer/miniAODeemm
// Class:      miniAODeemm
//
/**\class miniAODeemm miniAODeemm.cc ZeeYmmAnalyzer/miniAODeemm/plugins/miniAODeemm.cc
...
// #include "TROOT.h"
#include "TTree.h"
#include "ZeeYmmAnalyzer/miniAODeemm/plugins/miniAODeemm.h"

// class declaration
//
class miniAODeemm : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit miniAODeemm(const edm::ParameterSet &);
  ~miniAODeemm() override;
...
private:
  ...
  std::vector<float> *B_Z_TriggerPath, *B_Z_TriggerPath1;
  ...
  std::vector<float> *B_Z_Trigger32Pt1, *B_Z_Trigger32Eta1, *B_Z_Trigger32Phi1;
  std::vector<float> *B_Z_Trigger32Pt2, *B_Z_Trigger32Eta2, *B_Z_Trigger32Phi2;
  std::vector<float> *B_Z_Trigger32Pt3, *B_Z_Trigger32Eta3, *B_Z_Trigger32Phi3;
  std::vector<float> *B_Z_Trigger32Pt4, *B_Z_Trigger32Eta4, *B_Z_Trigger32Phi4;
  std::vector<float> *B_Z_Trigger32Pt5, *B_Z_Trigger32Eta5, *B_Z_Trigger32Phi5;
  ...
  std::vector<float> *B_Z_pt1, *B_Z_eta1, *B_Z_SCeta1, *B_Z_phi1, *B_Z_energy1, *B_Z_energyCorr1;
  ;

  std::vector<float> *B_Z_ecalIso1, *B_Z_hcalIso1, *B_Z_trackIso1;
  std::vector<bool> *B_Z_looseCutBase1, *B_Z_mediumCutBase1, *B_Z_mediumCutBase2, *B_Z_loose90wp1, *B_Z_mvaIsoWP90_1, *B_Z_looseLoosewp1;
  std::vector<float> *B_Z_mvaIsoWP80_1;
  std::vector<bool> *B_Z_loose1;
  std::vector<float> *B_Z_px2, *B_Z_py2, *B_Z_pz2;
  std::vector<float> *B_Z_pt2, *B_Z_eta2, *B_Z_SCeta2, *B_Z_phi2, *B_Z_energy2, *B_Z_energyCorr2;
  std::vector<float> *B_Z_ecalIso2, *B_Z_hcalIso2, *B_Z_trackIso2;
  std::vector<bool> *B_Z_looseCutBase2, *B_Z_loose90wp2, *B_Z_mvaIsoWP90_2, *B_Z_looseLoosewp2;
  std::vector<float> *B_Z_mvaIsoWP80_2;
...
miniAODeemm::miniAODeemm(const edm::ParameterSet &iConfig)
    : dimuon_Label(consumes<edm::View<pat::Muon>>(iConfig.getParameter<edm::InputTag>("dimuons"))),
      ...
      B_Z_loose90wp2(0),
      B_Z_mvaIsoWP90_2(0),
      B_Z_looseLoosewp2(0),  //B_Z_looseMva2(0),
      B_Z_mvaIsoWP80_2(0),
      B_Z_loose2(0),
      B_Z_charge1(0),
      B_Z_charge2(0),
...
void miniAODeemm::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  if (GenInfo == true) {
    ...
    for (size_t i = 0; i < pruned->size(); i++) {
      ...
      //cout<<"for loop interation: "<< i <<endl;
      //cout<<"pdg ID of particle: "<< (*pruned)[i].pdgId()<<endl;
      if (abs((*pruned)[i].pdgId()) == 11) {
        //cout<<"Found gen level muon"<<endl;
        cout << "Mother of 11: " << (*pruned)[i].mother()->pdgId() << endl;
      ...
      if (abs((*pruned)[i].pdgId()) == 13) {
        cout << "Mother of 13: " << (*pruned)[i].mother()->pdgId() << endl;
        //cout<<"Found gen level muon"<<endl;
        if ((*pruned)[i].mother()->pdgId() == 23) {
      ...
      if (abs((*pruned)[i].pdgId()) == 15) {
        //cout<<"Found gen level muon"<<endl;
        cout << "Mother of 15: " << (*pruned)[i].mother()->pdgId() << endl;
        if ((*pruned)[i].mother()->pdgId() == 23) {
          //cout<<"Found gen level muon from Jpsi "<<endl;
          //if ( (*pruned)[i].->pdgId()==10443 ) {
          //cout<<"Found gen level muon with number of mother"<<(*pruned)[i].numberOfMothers()<<endl;
    ...
    cout << "End Gen Work for this event number: " << nevent << " number of ZeeZuu: " << neeuu << " number of ZuuZuu: " << nuuuu
         << " number of ZeeZee: " << neeee << " number of ZttZuu: " << nttuu << " number of ZttZtt: " << ntttt << " number of ZttZee: " << nttee
         << endl;
    cout << endl;
  }

  // std::cout << "First work on Trigger " << std::endl;
  ...
  for (unsigned int i = 0, n = triggerBits->size(); i < n; ++i) {
    ...
    //cout<<acceptE<<endl;
    if (names.triggerName(i).find("HLT_Ele30_WPTight_Gsf_v") != string::npos) {
  ...
  if ((EET + EET1) > 0) {
    for (pat::TriggerObjectStandAlone obj : *triggerObjects) {  // note: not "const &" since we want to call unpackPathNames
      ...
      if (EET > 0) {
        if (obj.hasPathName("HLT_Ele30_WPTight_Gsf_v*", true, true) > 0) {
      ...
      for (View<pat::Electron>::const_iterator iEle1 = thePATElectronHandle->begin(); iEle1 != thePATElectronHandle->end(); ++iEle1) {
		...
		if (kfTrackRefM.isAvailable() && kfTrackRefM.isNonnull()) {
	      ...
	      */
          ...
          reco::TransientTrack electron1TT;
          reco::TransientTrack electron2TT;

          muon1TT = theB.build(glbTrackP);
          muon2TT = theB.build(glbTrackM);
          electron1TT = theB.build(iEle1->gsfTrack());
          electron2TT = theB.build(iEle2->gsfTrack());
	      ...
	      */
          ...
          B_Z_px->push_back(isZeeZuu);
          B_Z_py->push_back(neeuu);
          ...
          B_Z_looseCutBase1->push_back(iEle1->electronID("cutBasedElectronID-RunIIIWinter22-V1-loose"));
          B_Z_mediumCutBase1->push_back(iEle1->electronID("cutBasedElectronID-RunIIIWinter22-V1-medium"));
          ...
          B_Z_mvaIsoWP90_1->push_back(iEle1->electronID("mvaEleID-RunIIIWinter22-iso-V1-wp90"));
          ...
          B_Z_looseCutBase2->push_back(iEle2->electronID("cutBasedElectronID-RunIIIWinter22-V1-loose"));
          B_Z_mediumCutBase2->push_back(iEle2->electronID("cutBasedElectronID-RunIIIWinter22-V1-medium"));
          ...
          B_Z_mvaIsoWP90_2->push_back(iEle2->electronID("mvaEleID-RunIIIWinter22-iso-V1-wp90"));
  ...
  B_Z_loose90wp1->clear();
  B_Z_mvaIsoWP90_1->clear();
  B_Z_looseLoosewp1->clear();
  ...
  B_Z_loose90wp2->clear();
  B_Z_mvaIsoWP90_2->clear();
  B_Z_looseLoosewp2->clear();
...
void miniAODeemm::beginJob() {
  ...
  //tree_->Branch("B_isZeeZuu", &B_isZeeZuu);
  ...
  tree_->Branch("B_Z_loose90wp1", &B_Z_loose90wp1);
  tree_->Branch("B_Z_mvaIsoWP90_1", &B_Z_mvaIsoWP90_1);
  tree_->Branch("B_Z_looseLoosewp1", &B_Z_looseLoosewp1);
  ...
  tree_->Branch("B_Z_loose90wp2", &B_Z_loose90wp2);
  tree_->Branch("B_Z_mvaIsoWP90_2", &B_Z_mvaIsoWP90_2);
  tree_->Branch("B_Z_looseLoosewp2", &B_Z_looseLoosewp2);
...
void miniAODeemm::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
