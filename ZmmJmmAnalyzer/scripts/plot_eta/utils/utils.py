...
def read_root_file(file_path, cuts):
    ...
    # branches_to_read = ["Run", "LumiBlock", "B_J1_mass", "B_Mu1_pt", "B_Mu2_pt", "B_Mu1_eta", "B_Mu2_eta"]
    branches_to_read = ["B_Mu1_pt", "B_Mu2_pt", "B_Mu1_eta", "B_Mu2_eta"]
    arrays = tree.arrays(branches_to_read, library="np")

    # Apply cuts (e.g., for mass and other properties)
    # mask = np.ones(len(arrays["B_Mu1_pt"]), dtype=bool)
    ...
    # mu1_pt = arrays["B_Mu1_pt"][final_mask].astype(float)
    # mu2_pt = arrays["B_Mu2_pt"][final_mask].astype(float)
    # mu1_eta = arrays["B_Mu1_eta"][final_mask].astype(float)
    # mu2_eta = arrays["B_Mu2_eta"][final_mask].astype(float)
    # # event = arrays["Event"][final_mask].astype(int)
    return mu1_pt, mu2_pt, mu1_eta, mu2_eta
